<?php
    ...
    $select_driver = "SELECT `driver_id` FROM `offense_list`";
    $result = $conn->query($select_driver);

    while($row = mysqli_fetch_assoc($result)) {

        $get_area_name = "SELECT `meta_value` FROM `drivers_meta` WHERE driver_id = {$row['driver_id']} AND `meta_field` = 'present_address' LIMIT 4";
